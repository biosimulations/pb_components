...
from simularium_readdy_models.actin import ActinSimulation, ActinGenerator, FiberData
from simularium_readdy_models.common import get_membrane_monomers

from pb_multiscale_actin.create_readdy_pbif import generate_readdy_pbg, register_items_into_core
from pb_multiscale_actin.library.utils import simulate_readdy, id_difference
...
def get_test_monomers():
    actin_points = [np.array([-25, 0, 0]), np.array([25, 0, 0])]
    actin_monomers = ActinGenerator.get_monomers(
        fibers_data=[
            FiberData(fiber_id=28, points=actin_points, type_name="Actin-Polymer")
        ],
        use_uuids=False,
    ...
    num_actin_monomers = len(actin_monomers["particles"].keys())
    membrane_monomers = get_membrane_monomers(
        center=np.array([25.0, 0.0, 0.0]),
        size=np.array([0.0, 100.0, 100.0]),
        particle_radius=25,
        start_particle_id=num_actin_monomers,
        top_id=1,
    )
    num_membrane_monomers = len(membrane_monomers["particles"].keys())
    free_actin_monomers = ActinGenerator.get_free_actin_monomers(
        ...
        start_particle_id=num_actin_monomers
                          + num_membrane_monomers,
        start_top_id=2,
    )

    interested_monomers = [actin_monomers, membrane_monomers, free_actin_monomers]
    for im in interested_monomers:
        for k in im["particles"].keys():
            pos = im["particles"][k]["position"]
            im["particles"][k]["position"] = tuple(pos.tolist())

    monomers = {
        "particles": {**actin_monomers["particles"], **membrane_monomers["particles"], **free_actin_monomers["particles"]},
        "topologies": {
...
def test_readdy_actin_model() -> None:
    ...
    ReaddyUtil.add_monomers_from_data(readdy_simulation, get_test_monomers())
